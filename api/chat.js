// api/chat.js - Consolidated Chat API
...
module.exports = (req, res) => {
  ...
  console.log('[chat.js]', req.method, req.url, 'body:', JSON.stringify(req.body));

  // Health check - no auth required
  if (req.url === '/api/chat' || req.url === '/api/chat/') {
    return res.json({ status: 'ok', endpoints: ['messages','channels','communities','community-members','online-members','members','conversations'] });
  }
  ...
  authMiddleware(req, res, async () => {
    try {
      ...
      // Handle cases where Vercel passes different URL formats
      if (!path.startsWith('/api/')) path = '/api' + path;

      // Determine the endpoint. When Vercel routes /api/messages -> /api/chat,
      // req.url becomes /api/chat. So we use the 'endpoint' query param or body field.
      var endpoint = req.query?.endpoint || body.endpoint || path.replace('/api/', '');
      console.log('[chat.js] endpoint:', endpoint, 'method:', method, 'path:', path);

      if (endpoint === 'messages' || path === '/api/messages') {
        if (method === 'GET') {
        ...
        if (method === 'POST') {
          ...
          console.log('[chat.js] POST message:', { channel_id, content: content?.substring(0, 50), userId });
          ...
          if (error) { console.log('[chat.js] POST error:', error.message); return res.status(500).json({ error: error.message }); }
          if (type === 'dm') {
      ...
      if (endpoint === 'channels' || path === '/api/channels') {
      ...
      if (endpoint === 'communities' || path === '/api/communities') {
        if (method === 'GET') {
      ...
      if (endpoint === 'community-members' || path === '/api/community-members') {
        if (method === 'GET') {
      ...
      if (endpoint === 'online-members' || path === '/api/online-members') {
        if (method === 'POST') {
      ...
      if (endpoint === 'members' || path === '/api/members') {
        if (method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });
      ...
      if (endpoint === 'conversations' || path === '/api/conversations') {
        if (method !== 'GET') return res.status(405).json({ error: 'Method not allowed' });
      ...
      console.log('[chat.js] No route matched:', path);
